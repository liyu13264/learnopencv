...
def get_distort_arg():
    ...
    return '\'0,0 ' + str(np.random.randint(0, amount)) + ',' + str(np.random.randint(0, amount)) + ' 100,0 ' + str(
        np.random.randint(hundred_minus_amount, 100)) + ',' + str(np.random.randint(0, amount)) + ' 0,100 ' + str(
        np.random.randint(0, amount)) + ',' + str(np.random.randint(hundred_minus_amount, 100)) + ' 100,100 ' + str(
        np.random.randint(hundred_minus_amount, 100)) + ',' + str(np.random.randint(hundred_minus_amount, 100)) + '\''
...
def create_random_crops(image_filename, num_crops, out_dir):
    ...
    for i in range(0, num_crops):
        ...
        command = "magick convert " + image_filename + " -crop 32x32" + "+" + str(x) + "+" + str(
            y) + " " + os.path.join(out_dir, outfile)
...
for i in range(0, len(char_list)):
    ...
    for j in range(0, NUM_IMAGES_PER_CLASS):
        ...
        command = "magick convert " + str(list_filernd) + " -fill " + str(color) + " -font " + \
                  str(list_rfo) + " -weight 200 -pointsize 24 -distort Perspective " + str(
            distort_arg) + " " + "-gravity center" + " -blur 0x" + str(blur) \
                  + " -evaluate Gaussian-noise " + str(noise) + " " + " -annotate +" + x + "+" + y + " " + str(
            char_list[i]) + " " + char_output_dir + "output_file" + str(i) + str(j) + ".jpg"
